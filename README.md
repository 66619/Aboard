...
### 📏 教具功能
- **直尺工具**：可在画布上插入直尺，支持四边（上下左右）沿边缘画直线
- **三角板工具**：可在画布上插入三角板（SVG格式），画直线和角度（宽高比为√3:1）
- **沿边缘画线**：当笔触碰到教具图片边框时，自动沿边框画直线，不能在教具内部留下痕迹
- **自由操作**：支持拖动移动位置、旋转角度、调整大小
- **缩放适配**：教具在画布缩放后仍能正确移动和显示，不会异常放大或位置偏移
...
### 🖊️ 笔触差异化
- **圆珠笔**：根据书写速度模拟压力，慢速写粗、快速写细
- **毛笔**：带有毛边效果和笔锋感觉，模拟真实毛笔书写体验
- **铅笔**：半透明效果，模拟铅笔质感
- **钢笔**：标准书写笔触，清晰流畅
